...
def text():
	...
	w_labels = range(1, longest_word+1)
	s_group_size = 5
	s_label_ref_dict = {"{0} - {1}".format((int(x)-1)*s_group_size+1,int(x)*s_group_size):range((int(x)-1)*s_group_size+1,int(x)*s_group_size+1) for x in range(1, int(math.ceil(longest_sentence/float(s_group_size))+1))}
	s_labels = [x for x,y in sorted(s_label_ref_dict.iteritems(), key=itemgetter(1))]
	
	sentence_dump = [[sentence_length(sent), url, sent] for [url, sent] in sentences if sentence_length(sent) >= 40]
	
	conn.close()
	return render_template('text.html', s_group_size = s_group_size, s_label_ref_dict = s_label_ref_dict, word_len_urls = word_len_urls, sent_len_urls = sent_len_urls, w_labels = w_labels, s_labels = s_labels,  url_stats = url_stats, sentences = sentence_dump)